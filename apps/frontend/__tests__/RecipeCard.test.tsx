...
describe('RecipeCard', () => {
  ...
  it('renders correctly as top card', () => {
    const { queryByTestId } = render(
      <RecipeCard recipe={mockRecipe} isTopCard={true} />
    );
  ...
  it('handles long titles and descriptions correctly', () => {
    const longRecipe: RecipeCardType = {
      ...mockRecipe,
      title: 'This is a very long recipe title that should be truncated after two lines of text to prevent overflow',
      description: 'This is an extremely long description that goes on and on and on and should also be truncated after two lines to maintain the card layout and prevent text from overflowing the card boundaries',
    };
  ...
  it('displays difficulty with correct color', () => {
    const difficulties: Array<RecipeCardType['difficulty']> = ['easy', 'medium', 'hard'];
  ...
  it('limits dietary tags display to 2 with overflow indicator', () => {
    ...
    const { getByText, queryByText } = render(<RecipeCard recipe={recipeWithManyTags} />);
