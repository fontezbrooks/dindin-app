...
- Package manager: !`which bun pnpm npm yarn | head -1`
...
- Update dependencies: `bun install` (unless --skip-install)
- Source environment: `.env` file if exists

### Step 1: Linting

- Check linter passes: `turbo lint`
...
- Run comprehensive build checks:
  ```
  turbo run build:types build:dist build:app generate:docs dev:run typecheck
  ```
...
- Run test coverage: `turbo run test:coverage`
...
- Lint packages: `turbo run lint:package lint:deps`
...
- Run `bun run lint` to format files
...
- Lints entire codebase via Turbo
